...
Stack<T>::Stack(const Stack<T>& copy)
{
    ...
    if (!copy.empty())
    {
        StackFramePtr<T> temp = copy.top; 
        StackFramePtr<T>  new_top; 
        try 
        {
...
template<class T>
StackFramePtr<T> Stack<T>:: peek() const
{
    return top; 
}
//reverses the order of the stack 
template<class T>
StackFramePtr<T> Stack<T>::reverse()
{
    if (!empty())
    {
